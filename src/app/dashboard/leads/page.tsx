...
export default function LeadsPage() {
  ...
  const sendEmail = async (lead: Lead) => {
    ...
    try {
      ...
      const updated = { ...lead, status: 'contacted' as LeadStatus, email_sent_at: new Date().toISOString() }
      setLeads(prev => prev.map(l => l.id === lead.id ? updated : l))
      if (showLeadDetail?.id === lead.id) setShowLeadDetail(updated)
        ...
        {(['all', ...statusOptions] as string[]).map(s => (
          <button
            ...
            className={cn(
              ...
              statusFilter === s ? 'bg-brand-600 text-white' : 'text-surface-500 hover:bg-surface-100 dark:hover:bg-surface-800'
            )}
          >
                ...
                {filteredLeads.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-16 text-center text-surface-400">
                      ...
                      <p className="font-medium">{leads.length === 0 ? 'No leads yet' : 'No leads match filters'}</p>
                      <p className="text-sm mt-1">{leads.length === 0 ? 'Add a lead or upload a CSV to get started' : 'Try a different search or status'}</p>
                    </td>
                  </tr>
                ) : filteredLeads.map(lead => (
                  <tr key={lead.id} className="table-row-hover" onClick={() => setShowLeadDetail(lead)}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          ...
                          className={cn('btn-ghost py-1.5 px-2 text-xs', lead.personalized_message ? 'text-green-600 dark:text-green-400' : 'text-surface-300 cursor-not-allowed')}
                        ...
                        <button onClick={() => deleteLead(lead.id)} className="btn-ghost py-1.5 px-2 text-xs text-red-500" title="Delete">
  ...
  const onDrop = useCallback((files: File[]) => {
    ...
    Papa.parse(files[0], {
      ...
      complete: (results) => {
        ...
        if (rows.length === 0) {
          toast.error('No valid rows found. CSV needs name + email columns.')
        } else {
          onImport(rows)
        }
        setUploading(false)
