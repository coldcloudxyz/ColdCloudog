  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      setStats({
        ...
        replies: replies || 0,
        meetings_booked: met,
        conversion_rate: total > 0 ? Math.round((met / total) * 100 * 10) / 10 : 0,
      })
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Leads', value: stats.total_leads.toLocaleString(), icon: Users, color: 'text-brand-600 dark:text-brand-400', bg: 'bg-brand-50 dark:bg-brand-950' },
            { label: 'Emails Sent', value: stats.emails_sent.toLocaleString(), icon: Mail, color: 'text-green-600 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-950' },
            { label: 'Replies', value: stats.replies.toLocaleString(), icon: MessageSquare, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950' },
            { label: 'Meetings Booked', value: stats.meetings_booked.toLocaleString(), icon: Calendar, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-950' },
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-3xl font-bold text-white">
              {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : `${stats.conversion_rate}%`}
            </div>
        ...
        <div className="lg:col-span-3 card p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="hidden sm:flex items-center gap-4 text-xs">
              {[{ color: 'bg-brand-500', label: 'Sent' }, { color: 'bg-cyan-400', label: 'Opened' }, { color: 'bg-green-400', label: 'Replied' }].map(({ color, label }) => (
          ...
          {loading ? (
            ...
            <div className="h-[200px] flex items-center justify-center text-surface-400 text-sm text-center px-4">
              No email data yet — send your first outreach to see activity here.
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData}>
                <defs>
                  {[{ id: 'sent', color: '#0ea5e9' }, { id: 'opened', color: '#22d3ee' }, { id: 'replied', color: '#4ade80' }].map(({ id, color }) => (
                ...
                <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, fontSize: 12, color: '#f1f5f9' }} labelStyle={{ color: '#94a3b8', fontWeight: 600 }} />
        ...
        <div className="lg:col-span-2 card p-5">
          ...
          {loading ? (
            ...
            <div className="py-8 text-center text-surface-400">
              ...
              <Link href="/dashboard/leads" className="text-xs text-brand-600 hover:underline mt-1 block">Add your first lead →</Link>
            </div>
          ) : (
                ...
                <div key={lead.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`badge ml-2 flex-shrink-0 ${getStatusColor(lead.status)}`}>{getStatusLabel(lead.status)}</span>
                    </div>
      ...
      <div className="mt-6 grid sm:grid-cols-3 gap-4">
        {[
          { icon: Zap, label: 'Generate AI Messages', desc: 'Personalize outreach for new leads', href: '/dashboard/leads', color: 'from-brand-500 to-cyan-500' },
          ...
          { icon: Activity, label: 'View Analytics', desc: 'Deep dive into performance data', href: '/dashboard/analytics', color: 'from-green-500 to-emerald-500' },
        ].map(({ icon: Icon, label, desc, href, color }) => (
          <Link key={label} href={href} className="card p-4 flex items-center gap-4 hover:shadow-md transition-all group">
...
function buildChartData(leads: { email_sent_at: string | null; email_opened_at: string | null; email_replied_at: string | null }[]) {
  ...
  leads.forEach(lead => {
    if (lead.email_sent_at) {
      const key = new Date(lead.email_sent_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
      if (days[key]) days[key].sent++
    }
    if (lead.email_opened_at) {
      const key = new Date(lead.email_opened_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
      if (days[key]) days[key].opened++
    }
    if (lead.email_replied_at) {
      const key = new Date(lead.email_replied_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
